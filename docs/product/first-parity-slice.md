...
- Every normalized spell has a stable static detail route with mana source formulas, ordered animation/impact presentation declarations, ordered buff-local descriptions and halo declarations, spell- and buff-local AI hints, loss-aware buff lifecycle/stacking parameters, signed direct and sight-radius modifiers, target/player hit buff event hooks, typed effect-list options, direct effect damage/scaling and duration metadata, controls, and buff-presence conditions, resolved or explicitly unresolved spell/stat/item targets, cycle-safe recursive effect relationships, item/spell/buff-hook/ability/monster/instability backlinks, source/file provenance, and attached diagnostics. Detailed presentation references stay hidden, source frame values do not claim timing semantics, and AI hints, list options, damage/scaling/duration fields, effect controls, and buff conditions remain direct source metadata rather than fabricated formulas, countdowns, targeting, selection, probability, resistance, ignition, eligibility, consumption, or scheduling behavior. Unsupported nested buff mechanics remain visible as diagnostics rather than fabricated behavior, and sight modifiers do not claim a final visibility formula. Resolved spell references on item, stat, encrustment, and monster pages link to these routes.
