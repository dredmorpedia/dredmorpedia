...
- Every normalized spell has a stable static detail route with mana source formulas, ordered buff declarations, loss-aware buff lifecycle/stacking parameters, signed buff modifiers, direct effects, resolved or explicitly unresolved spell/stat targets, cycle-safe recursive effect relationships, item/spell/ability/monster/instability backlinks, source/file provenance, and attached diagnostics. Unsupported nested buff mechanics remain visible as diagnostics rather than fabricated behavior. Resolved spell references on item, stat, encrustment, and monster pages link to these routes.
