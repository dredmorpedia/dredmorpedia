...
- Ability normalization retains starting/level position, signed damage/resistance/primary/secondary modifiers, and the legacy-supported direct event/activated spell triggers with chance, delay, duration, resistance, and taxonomy metadata. Static ability pages link their parent skill and resolved spells, keep dangling spell names unlinked, preserve numeric primary/secondary IDs without invented definitions, and expose source-located diagnostics for remaining unsupported constructs.
...
- Normalized artifact: 32,322 bytes.
...
- Synthetic skill/ability coverage includes resolved, dangling, and generic loadout choices; always/optional quantities; a starting and leveled ability; signed damage/resistance/primary/secondary modifiers; resolved and dangling activated spells; and a probabilistic melee event trigger. Desktop/mobile browser checks follow item→skill→ability→spell relationships in both directions, verify numeric stat-ID disclosure, and exercise keyboard navigation.

## Read-only official verification

The canonical `1.1.5 beta_preview` base-plus-three-expansion dataset produces 763 items, 57 active encrustments, and 2,767 search documents with 0 errors, 3,873 warnings, and 71 info records.

- Normalized artifact: 4,027,585 bytes.
- Search artifact: 1,235,035 bytes uncompressed.
- Diagnostics: 1,781,195 bytes.
...
- The latest production build, including all 374 recipe, 57 encrustment, 52 canonical skill, 352 ability, and 951 spell pages plus registered/source-ID aliases, completed in approximately 40.7 seconds and prerendered 2,606 static pages.
