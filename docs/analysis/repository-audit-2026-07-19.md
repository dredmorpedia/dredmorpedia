...
| No build, package manifest, fixtures, tests, or CI existed | Node `24.18.0`, pnpm, strict TypeScript, formatting, lint, deterministic generation, CI, 139 unit/artifact tests, and 34 desktop/mobile Playwright tests are established. | Resolved |
...
| Imported XML and source metadata had an unclear escaping boundary | The parser rejects DOCTYPE input, validates relative asset values before any root probing, checks filesystem references against real source roots, and emits strict normalized schemas. The web verifies safe route-field shapes and unique same-kind canonical/alias ownership, while React renders imported strings as escaped text. | Resolved for the current local-import model; continue defense-in-depth review |
...
- [`web-route-artifact-boundary-evidence-2026-07-29.md`](web-route-artifact-boundary-evidence-2026-07-29.md), and
...
2. **Content parity:** the current canonical measurement has no remaining item compatibility diagnostic. It retains 239 unsupported or partially supported spell constructs, 13 spell-requirement diagnostics, and 23 dangling references. Each supported spell family still needs source/legacy evidence, strict normalization, synthetic coverage, UI disclosure, and deterministic official verification.
