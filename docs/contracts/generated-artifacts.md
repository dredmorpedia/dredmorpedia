...
The `monsters` collection preserves taxonomy, zero-based source level, effective one-based dungeon depth, special classification, icon and palette metadata, fighter/rogue/wizard source levels, optional experience value, deterministic signed stat bonuses, and optional parent identity. Description, taxonomy, depth, icon, palette, and stat bonuses follow the verified nested-monster inheritance rules; a child's matching bonus overrides its parent while unrelated parent bonuses remain. Archetype levels and experience stay local to the child record. These are source values, not fabricated derived combat totals. Spell hooks, drops, AI, and remaining presentation/behavior elements continue to emit diagnostics until their own verified slices normalize them.
...
- Monster `depth`, `special`, palette metadata, `archetypeLevels`, `experienceValue`, and `modifiers` were added to version 3 under the additive rule. Current web consumers require and validate the richer records, so older local schema 3 artifacts must be regenerated.
