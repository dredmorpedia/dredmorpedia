...
The `skills` collection preserves archetype, icon, ordered ability IDs, and complete starting-loadout definitions. Each loadout retains an optional named item key/name/resolved `itemId`, optional generic item type, positive quantity, and whether it is always included. The compatibility `loadoutItemKeys` array remains derived from named entries. Generic type-only choices are not fabricated as items, and unresolved named items remain visible with diagnostics.

The `abilities` collection preserves its parent skill key and optional resolved `skillId`, non-negative progression level, starting-ability flag, deterministic signed modifiers, and deterministic spell triggers. Ability modifiers use the same `damage`, `resistance`, `primary`, and `secondary` shape as encrustments, including the source key and finite amount. Numeric primary/secondary source IDs remain explicit rather than being fabricated as standalone stat definitions. Ability triggers use the same event-kind/chance/delay/duration/resistance/taxonomy contract as item triggers and retain original spell names plus optional resolved `spellId`. The compatibility `spellKeys` and `spellIds` arrays remain derived from those triggers. Supported direct event hooks and activated `<spell>` records are normalized; remaining skill/ability recovery, flag, currency, dodge, and tag constructs stay source-located diagnostics until their semantics are modeled or explicitly deferred.
...
- Skill `loadouts` and ability `level`, `startSkill`, `modifiers`, and `triggers` were added to version 3 under the additive rule. Current web consumers require and validate the richer records, so older local schema 3 artifacts must be regenerated. Existing key/ID arrays remain for compatibility and deterministic query use.
