...
The `abilities` collection preserves its parent skill key and optional resolved `skillId`, non-negative progression level, starting-ability flag, and deterministic spell triggers. Ability triggers use the same event-kind/chance/delay/duration/resistance/taxonomy contract as item triggers and retain original spell names plus optional resolved `spellId`. The compatibility `spellKeys` and `spellIds` arrays remain derived from those triggers. Supported direct event hooks and activated `<spell>` records are normalized; other ability modifiers remain source-located diagnostics until their own contract is implemented.
...
- Skill `loadouts` and ability `level`, `startSkill`, and `triggers` were added to version 3 under the additive rule. Current web consumers require and validate the richer records, so older local schema 3 artifacts must be regenerated. Existing key/ID arrays remain for compatibility and deterministic query use.
