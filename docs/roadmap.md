...
Progress through 2026-07-21: dataset schema 3, search schema 1, output-manifest schema 2, source-manifest schema 2, guarded patch overlays, a version-scoped published-route registry, versioned provenance UI, deterministic search and item/recipe/encrustment/skill/ability/spell relationship queries, collision-safe canonical routes and alternate aliases, search/stat/recipe/encrustment/skill/ability/spell routes, and desktop/mobile browser coverage are implemented. Spell graph traversal stops cycles and repeated branches while retaining every direct edge and dangling target. Skill loadouts retain named/generic choice, quantity, and always/optional semantics; ability progression and supported spell-trigger events are linked. Broader relation APIs, dependency policy, and final foundation documentation remain.
...
Progress through 2026-07-21: a synthetic-backed search route, item/stat detail links, stat backlinks/provenance, explicit no-stat-definition behavior, static recipe details, and bidirectional crafted-by/used-to-craft links are implemented. Item quality passed a separate synthetic, official-data, artifact, patch, and responsive UI review. Item spell triggers normalize legacy type-specific/direct event shapes and resolve active spells. Encrustment entities now normalize tool, visibility, skill level, instability, applicability, ingredient links, signed direct modifiers, named power hooks, and appearance descriptors; the dataset-wide instability-effect pool is separately modeled with linked spells and an explicit boundary around unavailable selection semantics. Static spell details expose direct effects, cycle-safe recursive chains, diagnostics, and backlinks from items, spells, abilities, monsters, and shared instability effects. Static skill and ability details expose starting loadouts, progression, signed damage/resistance/primary/secondary modifiers, source flags/tags/recovery/currency values, supported spell-trigger events including dodge hooks, and bidirectional item/spell navigation. Monster profiles normalize dungeon depth/special classification, archetype levels, experience, palette metadata, inherited stat bonuses and AI casting chance, local loss-aware aggressiveness/span/invisible source metadata, exact one-in on-hit odds, cast hooks, direct named/type-driven drops, and parent/variant/spell/item relationships on static routes. The drop/backlink and AI-metadata slices passed separate code review with strict artifact shapes and desktop/mobile coverage. All measured official skill/ability child elements and the scoped monster core/spell-hook/drop/AI-metadata fields are normalized. The acceptance statement remains a draft, official stat definitions need an approved source, remaining monster AI/behavior metadata and independently verified derived-stat parity are next, and broader legacy comparisons remain.
