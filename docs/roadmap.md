...
Progress through 2026-07-21: dataset schema 3, search schema 1, output-manifest schema 2, source-manifest schema 2, guarded patch overlays, a version-scoped published-route registry, versioned provenance UI, deterministic search and item/recipe/encrustment/spell relationship queries, collision-safe canonical routes and alternate aliases, search/stat/recipe/encrustment/spell routes, and desktop/mobile browser coverage are implemented. Spell graph traversal stops cycles and repeated branches while retaining every direct edge and dangling target. Broader relation APIs, dependency policy, and final foundation documentation remain.
