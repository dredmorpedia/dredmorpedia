...
Progress through 2026-07-21: dataset schema 3, search schema 1, output-manifest schema 2, source-manifest schema 2, guarded patch overlays, a version-scoped published-route registry, versioned provenance UI, deterministic search and item/recipe/encrustment relationship queries, collision-safe canonical routes and alternate aliases, search/stat/recipe/encrustment routes, and desktop/mobile browser coverage are implemented. Broader relation APIs, dependency policy, and final foundation documentation remain.
