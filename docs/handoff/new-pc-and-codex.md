...
5. Normalize the deeper encrustment modifier and unstable-effect vocabulary. Basic encrustment identities, tool/skill/visibility/instability/applicability fields, ingredient links, static pages, and used-to-encrust backlinks are implemented and verified with synthetic plus read-only official data.
6. Treat the current 4,421 unsupported/partially-supported element diagnostics, 16 dangling item references, and explicit encrustment effect diagnostics as the measured compatibility backlog, not as silently completed parity.
