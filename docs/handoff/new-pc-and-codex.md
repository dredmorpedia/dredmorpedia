...
5. Model the database-wide shared unstable-effect spell pool and determine its selection semantics without incorrectly attaching every definition to each encrustment. Direct signed modifiers, named power hooks, appearance descriptors, basic fields, ingredient links, static pages, and used-to-encrust backlinks are implemented and verified with synthetic plus read-only official data.
6. Treat the current 4,274 unsupported/partially-supported element diagnostics, 16 dangling item references, and one explicit shared unstable-effect diagnostic as the measured compatibility backlog, not as silently completed parity.
