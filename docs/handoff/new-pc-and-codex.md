...
4. No measured item, spell-child/effect, or spell-requirement compatibility diagnostic remains. Root spell auditing now exposes only one unsupported `noanimation` attribute for a separate feature decision; the targeting-template family is complete with all 106 active references resolved, all four measured `radius` declarations preserve exact non-negative source values, all three measured `self` declarations preserve strict source-binary flags, all 133 measured source-candidate `downtime` declarations are recognized with 131 effective active values, all 40 source-candidate `attack` declarations are recognized with 39 effective active true flags, all 71 source-candidate mine declarations are recognized with 70 effective active records, all 11 source-candidate `consumeItem` declarations are recognized with seven exact optional `consumeItemType` tokens, and all 192 source-candidate `wand` declarations are recognized with 191 effective active strict binary flags. The three exact shield requirements, one exact weapon requirement, six exact booze requirements, and three exact zorkmid requirements are preserved. Treat the 18 reviewed source-only records, one reviewed correction, and four remaining dangling references as explicit parity evidence. No measured official skill/ability or monster child element remains unsupported.
...
If first-parity approval remains deferred, classify the remaining root spell
`noanimation` attribute as a separate coherent slice before returning to the
preserved Meta view's "Required Armour by Monster" heuristic. Discuss that
formula with the owner immediately before implementing or intentionally
excluding it; do not present it as engine truth by default.
...
Only one root `noanimation` compatibility warning remains. Evidence is in
