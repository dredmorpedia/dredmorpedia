...
- The route-registry foundation follows `af3be37` (`feat: add guarded patch provenance`). Use `git log` to confirm the latest live branch state.
...
5. Complete remaining item/relationship fields using synthetic fixtures first. Deterministic slug handling, source/patch contracts, a durable published-route migration registry, and dataset/source/patch provenance UI are implemented locally.
