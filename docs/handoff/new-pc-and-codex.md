...
5. Continue monster parity: static core-profile routes, inheritance, spell/on-hit hooks, direct drops, reciprocal spell/item backlinks, and local AI aggressiveness/span source metadata are implemented; normalize remaining AI/behavior metadata next, then only independently verified derived-stat formulas.
...
Feature review status: the monster-drop/item-backlink and AI source-metadata slices are implemented and verified, but both need separate feature code review before they are treated as accepted parity.

Static monster details now expose taxonomy, dungeon-depth/special classification, fighter/rogue/wizard source levels, experience, palette metadata, effective inherited stat bonuses and AI casting chance, local AI aggressiveness/span source metadata, resolved or dangling on-hit/cast spell hooks, direct named/type-driven drops, parent/direct-variant navigation, provenance, and diagnostics. Exact one-in odds are retained alongside a rounded display percentage; resolved spells and named items link both ways. Drops and AI source metadata remain local to the declaring monster, and type-driven drops do not fabricate items. Derived combat totals remain explicitly unavailable until their formulas are independently verified. Synthetic desktop/mobile, keyboard, and axe coverage follows a child to its parent, spell, and drop item, verifies reciprocal item backlinks and inherited/overridden fields, exercises a generic artifact drop, displays AI metadata without claiming a behavior formula, and exposes dangling spell/item names without fabricating routes. The ignored official dataset contains 183 monsters: 132 inherit, 23 are special, all 183 resolve a dungeon depth, 177 supply experience, and 131 supply palette metadata. Inheritance produces 1,331 effective modifiers (458 damage, 486 resistance, 12 primary, and 375 secondary). Across 116 monsters, 274 hooks normalize (223 aware-casting and 51 on-hit); 271 resolve and three remain explicit dangling references, with at most 16 hooks on one monster. Fifty-six direct drops normalize across 15 monsters: all 49 named items resolve, and six artifact plus one zorkmid type-driven drops remain explicit, with at most 12 drops on one monster. All 183 routes export within 2,789 static pages. The remaining 432 unsupported monster elements cover partially modeled AI, sprite/sound, and behavior constructs for subsequent slices.
