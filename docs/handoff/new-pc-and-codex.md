...
The review determinism/test-hardening checkpoint gives generated search documents a total `(kind, name, id)` order and covers equal-precedence source resolution, missing monster parents, three-level inheritance, and genuinely negative derived primary totals. It closes the review's search-ordering and domain-coverage actions without changing the current official search order or UI. `pnpm.cmd check` passes all 128 unit/artifact tests and the 43-page synthetic export; deterministic official generation remains byte-identical with 0 errors, 275 warnings, and 71 informational decisions. Route-registry lifecycle policy and ICU-independent output ordering remain open medium findings.
