...
5. Implement monster parity next: expand the currently minimal monster inheritance/stat/spell/drop model, add static monster routes and backlinks, and retain unresolved or unsupported source constructs as diagnostics rather than inventing behavior.
6. Treat the current 3,836 unsupported/partially-supported element diagnostics and 16 dangling references as the measured compatibility backlog, not as silently completed parity. Items, spells, and monsters account for that remaining element backlog. The 16-entry shared instability pool is implemented and fully resolves in the canonical dataset; its source does not define weights, per-encrustment assignments, trigger rules, or a complete risk formula.
