...
5. Continue monster parity: static core-profile routes, inheritance, spell/on-hit hooks, and reciprocal spell backlinks are implemented; normalize drops and item backlinks next, then remaining AI/behavior metadata and only independently verified derived-stat formulas.
6. Treat the current 3,126 unsupported/partially-supported element diagnostics and 19 dangling references as the measured compatibility backlog, not as silently completed parity. Items account for 1,510 unsupported entries, spells for 1,169, and monsters for 447; three of the dangling references are genuine monster spell names absent from the active spell dataset. The 16-entry shared instability pool is implemented and fully resolves in the canonical dataset; its source does not define weights, per-encrustment assignments, trigger rules, or a complete risk formula.
...
## Monster profile and spell-hook slices completed

Static monster details now expose taxonomy, dungeon-depth/special classification, fighter/rogue/wizard source levels, experience, palette metadata, effective inherited stat bonuses and AI casting chance, resolved or dangling on-hit/cast spell hooks, parent/direct-variant navigation, provenance, and diagnostics. Exact one-in odds are retained alongside a rounded display percentage; resolved spells link both ways. Derived combat totals remain explicitly unavailable until their formulas are independently verified. Synthetic desktop/mobile, keyboard, and axe coverage follows a child to its parent and spell, verifies inherited/overridden bonuses and casting chance, and exposes a dangling hook without fabricating a route. The ignored official dataset contains 183 monsters: 132 inherit, 23 are special, all 183 resolve a dungeon depth, 177 supply experience, and 131 supply palette metadata. Inheritance produces 1,331 effective modifiers (458 damage, 486 resistance, 12 primary, and 375 secondary). Across 116 monsters, 274 hooks normalize (223 aware-casting and 51 on-hit); 271 resolve and three remain explicit dangling references, with at most 16 hooks on one monster. All 183 routes export within 2,789 static pages. The remaining 447 unsupported monster elements cover partially modeled AI, drops, sprite/sound, and behavior constructs for subsequent slices.
