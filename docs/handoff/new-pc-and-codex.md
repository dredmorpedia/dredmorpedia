...
- Synthetic desktop/mobile keyboard and axe checks pass. At the latest checkpoint, all 38 browser tests remain the interaction baseline, while 250 unit/artifact tests, deterministic official dataset/item-icon generation, and the complete 2,981-page local official export pass. The official artifact contains 2,829 searchable entities, including 62 project-authored stat definitions, and all 4,309 modifier declarations link to those definitions by exact selector. Search offers 61 used stat facets across 1,350 item, ability, spell, and encrustment records while keeping its 1,477,801-byte artifact inside ADR 0003's unchanged budgets. `/dataset/` makes its 5 ordered sources, 94 diagnostics, 36 affected records, and 71 override steps discoverable from the verified artifact set. Read-only full-dataset import/build/query measurements are recorded without the local installation path or official content.
...
If first-parity approval remains deferred, the next Phase 4 decision boundary
is the preserved Meta view's “Required Armour by Monster” heuristic. Discuss
that formula with the owner immediately before implementing or intentionally
