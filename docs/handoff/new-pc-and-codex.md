...
5. Continue monster parity: static core-profile routes and inheritance are implemented; normalize monster spell/on-hit hooks and add spell backlinks next, then drops/item backlinks, AI/behavior metadata, and only independently verified derived-stat formulas.
6. Treat the current 3,257 unsupported/partially-supported element diagnostics and 16 dangling references as the measured compatibility backlog, not as silently completed parity. Items account for 1,510 entries, spells for 1,169, and monsters for 578. The 16-entry shared instability pool is implemented and fully resolves in the canonical dataset; its source does not define weights, per-encrustment assignments, trigger rules, or a complete risk formula.
...
## Monster core-profile slice completed

Static monster details now expose taxonomy, dungeon-depth/special classification, fighter/rogue/wizard source levels, experience, palette metadata, effective inherited stat bonuses, parent/direct-variant navigation, provenance, and diagnostics. Derived combat totals remain explicitly unavailable until their formulas are independently verified. Synthetic desktop/mobile, keyboard, and axe coverage follows a child to its parent and verifies inherited/overridden bonuses. The ignored official dataset contains 183 monsters: 132 inherit, 23 are special, all 183 resolve a dungeon depth, 177 supply experience, and 131 supply palette metadata. Inheritance produces 1,331 effective modifiers (458 damage, 486 resistance, 12 primary, and 375 secondary). All 183 routes export within 2,789 static pages. The remaining 578 monster diagnostics identify spell/on-hit, drop, AI, sprite/sound, and behavior constructs for subsequent slices.
