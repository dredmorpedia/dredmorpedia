...
- Synthetic desktop/mobile keyboard and axe checks pass. At the latest checkpoint, all 38 browser tests remain the interaction baseline, while 255 unit/artifact tests, deterministic official dataset/item-icon generation, and the complete 2,981-page local official export pass. The official artifact contains 2,829 searchable entities, including 62 project-authored stat definitions, and all 4,309 modifier declarations link to those definitions by exact selector. Search offers 61 used stat facets across 1,350 item, ability, spell, and encrustment records. Its deterministic compact representation is 1,180,204 bytes uncompressed, 196,345 gzip, and 143,207 Brotli, restoring 319,796 raw bytes of ADR 0003 headroom without a schema or behavior change. Root spell auditing now leaves 919 unsupported attributes across 19 case-insensitive families; all 106 template-spell references and their loss-aware anchor flags are preserved and linked, 131 active spells expose exact source cooldown turns, and 39 expose the melee-attack source flag. `/dataset/` makes its 5 ordered sources, 1,013 diagnostics, 312 affected records, and 71 override steps discoverable from the verified artifact set. Read-only full-dataset import/build/query measurements are recorded without the local installation path or official content.
...
4. No measured item, spell-child/effect, or spell-requirement compatibility diagnostic remains. Root spell auditing now exposes 919 unsupported attributes across 19 case-insensitive families for separate feature decisions; the targeting-template family is complete with all 106 active references resolved, all 133 measured source-candidate `downtime` declarations are recognized with 131 effective active values, and all 40 source-candidate `attack` declarations are recognized with 39 effective active true flags. The three exact shield requirements, one exact weapon requirement, six exact booze requirements, and three exact zorkmid requirements are preserved. Treat the 18 reviewed source-only records, one reviewed correction, and four remaining dangling references as explicit parity evidence. No measured official skill/ability or monster child element remains unsupported.
