...
5. Complete encrustment relationships using synthetic fixtures first. Item quality has passed the separate review described below, and item spell triggers now normalize, resolve, and render with explicit unresolved states. Deterministic slugs, source/patch contracts, the route registry, item/recipe relationship queries, static recipe pages, and bidirectional crafting backlinks are also implemented locally.
