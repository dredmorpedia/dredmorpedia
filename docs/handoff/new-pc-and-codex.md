...
- Synthetic desktop/mobile keyboard and axe checks pass. At the latest checkpoint, all 36 browser tests remain the interaction baseline, while 179 unit/artifact tests, deterministic official generation, and the complete 2,857-page local official export pass. Read-only full-dataset import/build/query measurements are recorded without the local installation path or official content.
