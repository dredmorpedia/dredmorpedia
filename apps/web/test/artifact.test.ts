...
describe("generated artifact loading", () => {
  ...
  it("rejects malformed root spell self metadata", async () => {
    const artifact = readJson("artifact.json");
    const typedArtifact = artifact as unknown as {
      entities: {
        spells: { sourceSelfFlag: boolean | string | null }[];
      };
    };
    typedArtifact.entities.spells[0]!.sourceSelfFlag = "yes";
    writeOutput("artifact.json", artifact, true);
    const { loadArtifact } = await import("../src/lib/artifact");

    expect(() => loadArtifact()).toThrow(/sourceSelfFlag/);
  });

  it("rejects malformed root spell no-animation metadata", async () => {
    const artifact = readJson("artifact.json");
    const typedArtifact = artifact as unknown as {
      entities: {
        spells: { sourceNoAnimationFlag: boolean | string | null }[];
      };
    };
    typedArtifact.entities.spells[0]!.sourceNoAnimationFlag = "yes";
    writeOutput("artifact.json", artifact, true);
    const { loadArtifact } = await import("../src/lib/artifact");

    expect(() => loadArtifact()).toThrow(/sourceNoAnimationFlag/);
  });
  ...
  it("rejects a search document without its source-skill field", async () => {
    const search = readJson("search.json") as {
      documents: { craftingSkillLevel?: number | null }[];
    };
    const firstDocument = search.documents[0];
    if (!firstDocument) {
      throw new Error(
    ...
    delete firstDocument.craftingSkillLevel;
    writeOutput("search.json", search, true);
    const { loadSearchArtifact } = await import("../src/lib/artifact");

    expect(() => loadSearchArtifact()).toThrow(
      /documents\.0\.craftingSkillLevel/,
    );
  });
