...
The repository has completed its architecture spike and is building the first parity foundation. The legacy application remains isolated and runnable, while a strict TypeScript workspace provides deterministic synthetic XML import, versioned source/patch provenance and normalized/search artifacts, collision-safe static entity routes, explicit relationship backlinks, a version-scoped canonical-route/alias registry, shareable structured search, a static dataset-health/source-decision explorer, and desktop/mobile accessibility checks. No proprietary official game data is required or committed.
